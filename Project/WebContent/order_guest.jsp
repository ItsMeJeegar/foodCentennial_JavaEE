...
<body style="font-family: sans-serif;">
	...
	<div class="content">
		...
		<div style="width: 1000px; height: 50px; float: left;">
			<h2 style="margin-left: 50px;">GUEST ORDER</h2>
		</div>
		...
		<div
			...
			int oc = 0;
				Connection conn = null;
					...
					try {
						String sql = "select * from OrderMain where orderNumber = '"+session.getAttribute("ordernumber").toString()+"'";
			...
			<table
				...
				<%
					stmt = conn.createStatement();
								String odsql = "select * from OrderDetail where orderNumber = " + rs.getString("orderNumber")
										+ " order by restaurant";
				...
				<tr>
					...
					<%
											...
											<td
												style="border: 1px solid #bbbbbb; width: 250px; padding: 5px; color: black;">
												<%=od.getString("orderStatus").toString()%></td>
											<%
						}
					%>
...
<h3 style="line-height: 50px;">Sorry! No order with order number <%=session.getAttribute("ordernumber").toString() %> was found! <br>
</h3>
</div>
							<%
						}
